                    ...
                    if (err) reject(err);
                        ...
                        readStream.on('end', () => {
                            ...
                            if (parsedJSON) {
                                ...
                                try { repo[id].addVersion(parsedJSON) }
...
// TODO Make the fetch function use a download manager (as parameter)
const defaultFetchFunction = () => request.get(config.repository, { encoding: null });
...
export default class Repository {
    ...
    constructor(directories: DirectorySet, fetchDataFunction: () => Promise<Buffer> = defaultFetchFunction) {
    ...
    async fetch() {
        const { mods, cache } = await this._fetchDataFunction()
            .then(openArchive)
    ...
    async loadFromCache(): Promise<Array<Error>> {
        ...
        for (let modID in cache) {
            ...
            versions.forEach(version => {
                try { mod.addVersion(version) }
                catch (err) { parsingErrors.push(err) }
            });
            cache[modID] = mod;
        }
